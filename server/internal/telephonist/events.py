...
async def make_and_validate_event(
    ...
    if realtime.is_reserved_event(descriptor.name):
        raise HTTPException(422, f"event type '{descriptor.name}' is reserved for internal use")
...
async def get_sequence(sequence_id: PydanticObjectId, app_id: Optional[PydanticObjectId] = None):
    sequence = await EventSequence.get(sequence_id)
    if sequence is None:
        raise HTTPException(404, f"Sequence with id = {sequence_id} does not exist")
    if app_id and sequence.app_id != app_id:
        raise HTTPException(
            401,
            f"Sequence with id = {sequence_id} does not belong to application with id = {app_id}",
        )
    return sequence
...
async def finish_sequence(sequence: EventSequence, finish_request: FinishSequence, ip_address: str):
    ...
    events = [
        Event(
            ...
            event_key=f"{event_type}@{sequence.task_name}" if seq.task_name else event_type,
            publisher_ip=ip_address,
    ...
    if finish_request.error_message:
        _logger.warning(f"sequence {sequence.name} ({sequence.id}) errored: {body.error_message}")
