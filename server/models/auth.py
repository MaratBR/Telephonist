...
from server.internal.auth.utils import encode_token, decode_token, hash_password, create_static_key
...
class TokenModel(BaseModel):
    ...
    @validator('exp', 'nbf', 'iat', pre=True)
    def _dt_to_timestamp(cls, value: datetime):
...
class User(Document):
    ...
    def create_token(self, token_type: str = 'access', lifetime: Optional[timedelta] = None,
                     check_string: Optional[str] = None):
        if check_string:
            check_string = hashlib.sha256(check_string.encode()).hexdigest()
        return TokenModel(sub=self.id, token_type=token_type, is_superuser=self.is_superuser, username=self.username,
                          exp=datetime.now() + (lifetime or timedelta(hours=4)), check_string=check_string)
    ...
    async def create_user(
        ...
        user = cls(username=username, password_hash=hash_password(password), email=email, password_reset_required=password_reset_required)
        await user.save()
        return user
