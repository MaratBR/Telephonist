...
def get_user_token(client: TestClient):
    d = client.post('/auth/token', json={'login': settings.default_username, 'password': settings.default_password}).json()
    return d['access_token']


def test_create_application(client: TestClient):
    client.headers['authorization'] = 'Bearer ' + get_user_token(client)
    resp = client.post('/applications', json={'name': 'My application', 'description': 'This is a new application', 'tags': ['new', 'important']})
